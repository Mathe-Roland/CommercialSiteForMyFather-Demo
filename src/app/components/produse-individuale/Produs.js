...
const Produs=({img,description,title,price})=>{
    ...
    useEffect(() => {
        ...
        // Call the fetchDataAndFilter function
        fetchDataAndFilter();
    ...
    const handleUserData = async () => {
        try {
            ...
            const filteredSpecificPanouUserRelatedData = data.data.filter(
                (element) => 
                    {
                        return element.attributes.title === title
            ...
            if (filteredSpecificPanouUserRelatedData.length > 0) {
                if (filteredOptiuniNormale.length > 0) {
                    const newDatas = {
                        price: hadlePrice(selectedValues),
    ...
    return(
        ...
        <div className="produs-container">
            ...
            <p className="produs-pret">{ifVopsit ? prices :hadlePrice(selectedValues)}</p>
            </div>
        ...
        {commentList?.map((element)=>(
            ...
            panouId={panouId}
            user={username}
            username={element?.author?.name} comments={element.content}/>
        ))}
